...
GameToolApplication::GameToolApplication(
    ...
    appPipeName_(std::move(pipeName)),
...
bool GameToolApplication::registerApp() {
    ...
    if (!WriteFile(
        ...
        std::cerr << "Failed to send registration message: " << GetLastError() << std::endl;
        return false;
    }
...
bool GameToolApplication::connectToDock() {
    // 连接到Electron的主管道
    std::string mainPipeName = R"(\\.\pipe\electron_main_pipe)";
    ...
    if (WaitNamedPipe(mainPipeName.c_str(), 5000)) {
        ...
        if (mainPipeHandle_ != INVALID_HANDLE_VALUE) {
            // 启动读取线程
            mainPipeThread_ = std::thread(&GameToolApplication::mainPipeReadThread, this);
            return true;
        }
    }
...
bool GameToolApplication::startPipeServer() {
    ...
    // 创建命名管道服务器
    std::string fullPipeName = R"(\\.\pipe\)" + appPipeName_;
    ...
    // 启动服务器线程
    running_ = true;
    serverThread_ = std::thread(&GameToolApplication::pipeServerThread, this);
...
void GameToolApplication::pipeServerThread() {
    while (running_) {
        // 等待客户端连接
...
void GameToolApplication::mainPipeReadThread() {
    char buffer[4096];
    DWORD bytesRead;
    std::string messageBuffer;

    while (mainPipeHandle_ != INVALID_HANDLE_VALUE) {
        if (ReadFile(mainPipeHandle_, buffer, sizeof(buffer), &bytesRead, nullptr)) {
            if (bytesRead > 0) {
                // 将接收到的数据添加到消息缓冲区
                messageBuffer.append(buffer, bytesRead);
